...
# 新增：询问是否开启 PMTU 探测
inst_mtu(){
    green "请选择 MTU 探测策略 (隧道/中转建议关闭)："
    echo ""
    echo -e " ${GREEN}1.${PLAIN} 关闭探测 (推荐 IPv6 隧道/NAT 环境) ${YELLOW}（默认）${PLAIN}"
    echo -e " ${GREEN}2.${PLAIN} 开启探测 (标准直连环境)"
    echo ""
    read -rp "请输入选项 [1-2]: " mtuInput
    
    if [[ $mtuInput == 2 ]]; then
        disable_mtu="false"
        yellow "已选择：开启 MTU 探测 (disablePathMTUDiscovery: false)"
    else
        disable_mtu="true"
        yellow "已选择：关闭 MTU 探测 (disablePathMTUDiscovery: true) - 兼容隧道环境"
    fi
}
...
quic:
  ...
  disablePathMTUDiscovery: $disable_mtu
...
proxies:
  - name: Misaka-Hysteria2
    ...
    disable-mtu-discovery: $disable_mtu
proxy-groups:
  - name: Proxy
    type: select
    proxies:
